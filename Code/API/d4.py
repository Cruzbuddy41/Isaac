...
try:
    while True:
        ...
        big_v1 = [0, h]
        big_v2 = [w, h]
        big_v3 = [int(w * 0.8), int(h * 0.2)]
        big_v4 = [int(w * 0.2), int(h * 0.2)]
        big_pts = np.array([[big_v1, big_v2, big_v3, big_v4]], np.int32)

        small_v1 = [int(w * 0.35), int(h * 0.7)]
        small_v2 = [int(w * 0.65), int(h * 0.7)]
        small_v3 = [w // 2, int(h * 0.35)]
        ...
        big_lines = cv2.HoughLinesP(big_edges, 1, np.pi / 180, 30, minLineLength=40, maxLineGap=50)
        small_lines = cv2.HoughLinesP(small_edges, 1, np.pi / 180, 20, minLineLength=20, maxLineGap=50)
        ...
        elif big_right_line_detected or big_left_line_detected:
            direction = "FORWARD"
            movement.move_forward(40, 0.3)
        else:
            direction = "SEARCHING"
            movement.move_forward(35, 0.2)
